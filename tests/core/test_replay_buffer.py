...
def test_sac_buffer_batched_add():
    def create_batch(size, mult=1):
        obs = (mult * np.expand_dims(np.arange(100, 100 + size), axis=1) *
               np.ones((size, 2)).astype(np.int8))
        act = mult * np.expand_dims(np.arange(0, size), axis=1).astype(np.int8)
        next_obs = obs + act
        reward = mult * 10 * np.expand_dims(np.arange(0, size), axis=1).astype(
            np.int8)
        done = np.random.randint(0, 1, size=(size, 1), dtype=bool)
        return obs, act, next_obs, reward, done

    def compare_batch_to_buffer_slice(start_idx, batch_size, obs, act,
        ...
        for i in range(batch_size):
            ...
            np.testing.assert_array_equal(buffer.next_obses[buffer_idx],
                                          next_obs[i])
            np.testing.assert_array_equal(buffer.rewards[buffer_idx],
                                          reward[i])
            np.testing.assert_array_equal(buffer.not_dones[buffer_idx],
                                          np.logical_not(done[i]))
            np.testing.assert_array_equal(buffer.not_dones_no_max[buffer_idx],
                                          done[i])
    ...
    buffer.add_batch(obs_, act_, reward_, next_obs_, done_,
                     np.logical_not(done_))
    assert buffer.idx == batch_size_
    assert not buffer.full
    compare_batch_to_buffer_slice(0, batch_size_, obs_, act_, next_obs_,
                                  reward_, done_)
    ...
    assert buffer.idx == 0
    assert buffer.full
    compare_batch_to_buffer_slice(batch_size_, batch_size_, obs_, act_,
                                  next_obs_, reward_, done_)  # new additions
    compare_batch_to_buffer_slice(0, batch_size_, obs_, act_, next_obs_,
                                  reward_,
                                  done_)  # Check that nothing changed here
    ...
    compare_batch_to_buffer_slice(0, 11, obs_[16:], act_[16:], next_obs_[16:],
                                  reward_[16:], done_[16:])
    # Now check that the last 9 observations are correct
    compare_batch_to_buffer_slice(11, 9, obs_[7:16], act_[7:16],
                                  next_obs_[7:16], reward_[7:16], done_[7:16])
...
def test_partial_trajectory_overlaps():
    buffer = replay_buffer.ReplayBuffer(4, (1, ), (1, ),
...
def test_sample_trajectories():
    buffer = replay_buffer.ReplayBuffer(15, (1, ), (1, ),
                                        max_trajectory_length=10)
    dummy = np.zeros(1)
...
def test_get_last_n_trajectories():
    buffer = replay_buffer.ReplayBuffer(100, (1, ), (1, ),
                                        max_trajectory_length=10)

    assert buffer.get_last_n_trajectories(3) is None
    ...
    o, a, no, r, d = buffer.get_last_n_trajectories(3).astuple()
    assert len(o) == 7 * 3
    assert o[-1] == 9
    assert o[0] == 7
...
def test_transition_iterator():
    def _check_for_capacity_and_batch_size(num_transitions, batch_size):
        ...
        transitions = TransitionBatch(input_obs, dummy, input_obs + 1, dummy,
                                      dummy)
...
def test_transition_iterator_shuffle():
    def _check(num_transitions, batch_size):
        ...
        it = replay_buffer.TransitionIterator(transitions,
                                              batch_size,
        ...
        assert all(
            [a == b for a, b in zip(all_obs_sorted, range(num_transitions))])
...
def test_bootstrap_iterator():
    ...
    def _check(num_transitions, batch_size, permute):
        ...
        transitions = TransitionBatch(input_obs, dummy, input_obs + 1, dummy,
                                      dummy)
        it = replay_buffer.BootstrapIterator(transitions,
                                             batch_size,
                                             num_members,
                                             permute_indices=permute)
...
def test_sequence_iterator():
    ...
    for i in range(num_trajectories):
        ...
        for j in range(traj_length):
            v = F * i + j
            buffer.add(dummy * v, dummy * v + 1, dummy * v + 2, v,
                       j == traj_length - 1)
    ...
    def _check_non_ensemble_sequence_batch(batch_, expected_batch_size_):
        assert batch_.obs.shape == (expected_batch_size_, sequence_length, 1,
                                    1)
    ...
    def _do_test(batch_size_, sequence_length_, shuffle_each_epoch_,
                 ensemble_size_):
        ...
        for batch_idx, batch in enumerate(iterator):
            expected_batch_size = _expected_batch_size(batch_size_, batch_idx,
                                                       iterator)
            ...
            for e1 in range(ensemble_size_):
                ...
                if expected_batch_size == 8:
                    for e2 in range(e1 + 1, ensemble_size_):
                        assert not np.allclose(batch.obs[e1, :, 0],
                                               batch.obs[e2, :, 0])

                # Now check that each ensemble batch is consistent
                _check_non_ensemble_sequence_batch(batch[e1],
                                                   expected_batch_size)
        ...
        init_states_first_epoch = np.sort(obs_first_epoch[:, :, 0, :, :],
                                          axis=1)
        init_states_second_epoch = np.sort(obs_second_epoch[:, :, 0, :, :],
                                           axis=1)
