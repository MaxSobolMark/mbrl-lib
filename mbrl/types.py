...
class TransitionBatch:
    ...
    active_policy: Optional[TensorType]

    def __len__(self):
        return self.obs.shape[0]

    def astuple(self) -> Transition:
        return self.obs, self.act, self.next_obs, self.rewards, self.dones, self.active_policy

    def __getitem__(self, item):
        return TransitionBatch(self.obs[item], self.act[item],
                               self.next_obs[item], self.rewards[item],
                               self.dones[item], self.active_policy[item])
    ...
    def add_new_batch_dim(self, batch_size: int):
        if not len(self) % batch_size == 0:
            raise ValueError("Current batch of transitions size is not a "
        ...
        return TransitionBatch(
            ...
            self.next_obs.reshape(
                self._get_new_shape(self.obs.shape, batch_size)),
            self.rewards.reshape(
                self._get_new_shape(self.rewards.shape, batch_size)),
            self.dones.reshape(
                self._get_new_shape(self.dones.shape, batch_size)),
            self.active_policy.reshape(
                self._get_new_shape(self.active_policy.shape, batch_size)))
