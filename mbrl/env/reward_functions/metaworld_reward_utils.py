...
def _reward_pos(obs: torch.Tensor, goal_position: torch.Tensor,
    ...
    in_place = tolerance(
        torch.norm(hand - door - torch.Tensor([0.05, 0.03, -0.01]), dim=-1),
        bounds=(0, threshold / 2.0),
        margin=0.5,
        sigmoid='long_tail',
    )
    ready_to_open = hamacher_product(above_floor, in_place, device)
    ...
    opened = tolerance(
        ...
        torch.norm(door - goal_position, dim=-1),
        # bounds=(0, 0.5),
        bounds=(0, 0.08),
        # margin=np.pi/3.,
        margin=0.20,
...
def _reward_grab_effort(actions: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        tf.Tensor: [n]
    """
    return (torch.clamp(actions[:, 3], -1, 1) + 1.0) / 2.0
