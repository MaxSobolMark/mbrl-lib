...
class SawyerDrawerOpenEnvV2(SawyerXYZEnv):
    def __init__(self, task_number: int, num_tasks: int = 20):
    ...
    def evaluate_state(self, obs, action):
        (reward, gripper_error, gripped, handle_error, caging_reward,
         opening_reward) = self.compute_reward(action, obs)
    ...
    def reset_model(self):
        ...
        #     [.0, -.16 - self.maxDist, .09])
        # MODIFICATION
        factor = (self._num_tasks - (self._task_number - 1)) / self._num_tasks
        self._target_pos = self.obj_init_pos + (np.array(
