...
class SawyerFaucetCloseEnvV2(SawyerXYZEnv):
    def __init__(self, closing_angle: float = 0.):
        ...
        # MODIFICATION
        self._closing_angle = np.radians(closing_angle)
    ...
    def reset_model(self):
        ...
        self._target_pos = self.obj_init_pos + np.array([
            np.cos(self._closing_angle) * (-self._handle_length),
            np.sin(self._closing_angle) * (-self._handle_length), .125
        ])
